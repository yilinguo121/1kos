...
struct process *create_process(const void *image, size_t image_size) {
    ...
    // 设置被调用者保存的寄存器，这些寄存器值将在 switch_context 中的第一次上下文切换时被恢复。
    // 在这里，我们把返回地址设置为 user_entry，这样用户进程启动时将跳转到 user_entry。
    ...
    *--sp = (uint32_t) user_entry;  // ra (已更改!)
    ...
    // 映射用户页：如果传入了镜像数据，则将其加载到用户地址空间中
    if (image != NULL && image_size > 0) {
        for (uint32_t off = 0; off < image_size; off += PAGE_SIZE) {
            paddr_t page = alloc_pages(1);

            // 处理最后一页数据可能不足一个页面大小的情况
            size_t remaining = image_size - off;
            size_t copy_size = (PAGE_SIZE <= remaining) ? PAGE_SIZE : remaining;

            // 将镜像中当前页面的数据复制到新分配的物理页面上
            memcpy((void *) page, (const char *) image + off, copy_size);
            // 将该物理页面映射到用户空间的地址 USER_BASE + off，并设置用户、读、写、执行权限
